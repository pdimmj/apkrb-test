document.addEventListener('DOMContentLoaded', function() {
    ...
    function updatepphDuaDua() {
        const berNpwp = form.elements['npwp'].value;
        const dasarPengenaan = parseFloat(form.elements['dasarPengenaan'].value.replace(/,/g, ''));

        let pphDuaDua;

        if (dasarPengenaan <= 2000000) {
            pphDuaDua = 0;
        } else if (berNpwp === 'YA') {
            pphDuaDua = Math.round(dasarPengenaan * 0.015);
        } else {
            pphDuaDua = Math.round(dasarPengenaan * (0.015*2));
        }

        form.elements['pphDuaDua'].value = pphDuaDua.toString().replace(/\d(?=(\d{3})+$)/g, '$&,');
    }
