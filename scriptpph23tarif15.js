document.addEventListener('DOMContentLoaded', function() {
    ...
    function updatepphDuaTiga() {
        ...
        if (berNpwp === 'YA') {
            pphDuaTiga = Math.round(dasarPengenaan * 0.15);
        } else {
            pphDuaTiga = Math.round(dasarPengenaan * (0.15 * 2));
        }

        form.elements['pphDuaTiga'].value = pphDuaTiga.toString().replace(/\d(?=(\d{3})+$)/g, '$&,');
    }
